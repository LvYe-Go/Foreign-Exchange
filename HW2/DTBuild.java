...
public class DTBuild {
      public static void main(String[] args) throws IOException{
          ...
          TreeNode root =  genNode(DTUtli.TrainFileName, featureList);
       ...
       * @param filename:  fileame to readIn from testData
	  ...
	  */
      public static TreeNode genNextLevelNodes(String filename, String feature, String label) throws IOException {
    	  ...
    	  while ((str = bufferedReader.readLine()) != null) {
    		   if(isDirtData(str)) continue; // filter out the incomplete data
				...
				if(line[DTUtli.map.get(feature)].equals(label)){
	   ...
	   * give valeu to the new label;
	   * @param filename: filename for put in 
	   * @param featureList
	   * @return
	   * @throws IOException
	   */
	  public static TreeNode genNode(String filename, List<String> featureList) throws IOException{
		  if(featureList.size() == 1) {
			  ...
			  int splitIndex = getMaxIGIndex(filename, featureList);
			  TreeNode node = new TreeNode(DTUtli.count, featureList.get(splitIndex));
			  DTUtli.count++;
			  splitFiles(node, filename);
			  List<String> newFeatureList = new ArrayList<String>(featureList);
			  newFeatureList.remove(node.feature);  // remove the found feature 
			  node.left = genNode(DTUtli.FILE_PREFIX + node.num + DTUtli.IS_INCREASE, newFeatureList);
			  node.right = genNode(DTUtli.FILE_PREFIX + node.num + DTUtli.IS_DECREASE, newFeatureList);
			  return node;
		  }
	  }
       ...
       * @throws IOException
       */
  	public static int getMaxIGIndex(String filename, List<String> featureList) throws IOException{
		...
		while ((str = bufferedReader.readLine()) != null) {
			...
			total_line++;
			for(int i = 0; i < featureSize; i++){
				if(parts[cur_map.get(i)].equals(DTUtli.IS_INCREASE)){
		...
		// IG = H(Y)-H(Y|X_i), here H(Y) is same for them
  		...
  		for(int i=0; i < featureSize; i++){
            ...
            prob[i] = p_x0*(-p_x0_y0*Math.log(p_x0_y0) - p_x0_y1*Math.log(p_x0_y1))
                    + p_x1*(-p_x1_y0*Math.log(p_x1_y0) -  p_x1_y1*Math.log(p_x1_y1));
  		}
     ...
     */
  	...
  	public static void writeNewFile(TreeNode node, String filename) throws IOException {
			...
			while ((str = bufferedReader.readLine()) != null) {
				...
				String[] parts = str.split(" ");
				if(parts[DTUtli.map.get(node.feature)].equals(DTUtli.IS_INCREASE)){
